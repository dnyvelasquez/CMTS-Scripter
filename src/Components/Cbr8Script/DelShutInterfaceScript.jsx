...
const DelShutInterfaceScript = () => {
    ...
    const [scriptDelShutInterface, setScriptDelShutInterface] = useState("");
    const scriptArray = [];
    ...
    const script = () => {
        ...
        if (delShutInterface === 'shutdown') {
            ...
            setScriptDelShutInterface(scriptArray.join(''));  
        }
    
        if (delShutInterface === 'downstream') {
            scriptArray.push(`interfaceCable${context.interfaceCable}\n`)
            scriptArray.push(`  shutdown\n`);
            scriptArray.push(`  no downstream Integrated-Cable ${context.interfaceCable} rf-channel 0-47\n`);
            scriptArray.push(`exit\n`);
            context.script.scriptDelShutInterface = scriptArray.join('');  
            setScriptDelShutInterface(scriptArray.join(''));  
        }
        ...
        <div className="flex flex-col w-full h-full">
            <textarea
                ...
                onChange={(event) => {
                    ...
                    setScriptDelShutInterface(event.target.value);
                }}
            />
            <div className="flex justify-end gap-1">
                <select
                    ...
                    onChange={(event) => {setDelShutInterface(event.target.value);}}
                >
                ...
                <button 
                    className="primary-button border-2 w-20 h-8 rounded-md"
                    onClick={() => {copy(context.script.scriptDelShutInterface)}}
                >
                    Copy
                </button>
            </div>
